	...
	while (getline(objFile, str)) {
		...
		if (!splitted_line.empty()) {
			if (splitted_line[0] == "usemtl") {
				...
				currVertexDrawOrderList.push_back(stoi(split(splitted_line[1], '/')[0].c_str()));
				currVertexDrawOrderList.push_back(stoi(split(splitted_line[2], '/')[0].c_str()));
				currVertexDrawOrderList.push_back(stoi(split(splitted_line[3], '/')[0].c_str()));

				currNormalDrawOrderList.push_back(stoi(split(splitted_line[1], '/')[2].c_str()));
				currNormalDrawOrderList.push_back(stoi(split(splitted_line[2], '/')[2].c_str()));
				currNormalDrawOrderList.push_back(stoi(split(splitted_line[3], '/')[2].c_str()));
			}
		}
	}
	allVertexDrawOrderList.push_back(currVertexDrawOrderList);
